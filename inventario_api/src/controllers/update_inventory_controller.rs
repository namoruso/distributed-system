...
pub async fn change_by_id(Path((id,mode)): Path<(i32, String)>, payload: Result<Json<StockUpdate>, JsonRejection>) -> impl IntoResponse {
    
    if mode != "increase" && mode != "decrease" {
        eprintln!("{}: Error en el modo del cambio de stock",StatusCode::NOT_FOUND);
        return (StatusCode::NOT_FOUND, Json(json!({"message": "Error al obtener el modo"}))) 
    }
    ...
    let data = match result {
        Ok(record) => {
            ...
            if !record.status {
                return (StatusCode::NOT_FOUND, Json(json!({"message": "Producto inexistente"}))) 
            }
            record
        },
        Err(error) => {
    ...
    match mode.as_str() {
        "increase" => {
            ...
            if _new_stock > data.maximun {
                eprintln!("{}: Error en los datos",StatusCode::BAD_REQUEST);
                return (StatusCode::BAD_REQUEST, Json(json!({"message": "El stock no puede superar el maximo"})))
            }
        },
        "decrease" => {
            ...
            if _new_stock < data.minimun {
                eprintln!("{}: Error en los datos",StatusCode::BAD_REQUEST);
                return (StatusCode::BAD_REQUEST, Json(json!({"message": "No hay stock disponible"})))
            }
        },
        _ => {
            eprintln!("{}: Error en el modo del cambio de stock",StatusCode::NOT_FOUND);
